...
public:
    ...
    void print();
    // returns game status after action is performed (according to content of revealed cell)
    ...
    void reveal_all_cells() { for (auto &c : board) c.set_status(CellStatus::Revealed); }
private:
    ...
    const std::size_t count_nearby_mines(const std::size_t idx);
    void reveal_cell_bfs(const std::size_t idx);
};
